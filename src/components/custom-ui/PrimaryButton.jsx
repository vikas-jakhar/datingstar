const PrimaryButton = ({ text, hoverClass, className = '', ...props }) => {
    return (
        <button
            className={`overflow-hidden bg-off-yellow group relative rounded-xl z-[1] sm:text-base ${className}`}
            {...props}
        >
            <span className="group-hover:drop-shadow-gray-sm font-raleway transition_slow font-semibold text-base text-black">{text}</span>
